...
from wp.v3.v16_policy import benjamini_hochberg, policy_metrics
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run V18 causal-persistence ranking with separately calibrated "
            "candidate-day frequency thresholds."
        )
    )
...
def main() -> int:
    ...
    for fold in folds:
        ...
        print(
            ...
            f"model_calibration={model_calibration_dates[0]}.."
            f"{model_calibration_dates[-1]} "
            f"rows={len(model_calibration):,} "
        ...
        bundle = fit_ranked_selector(
            train,
            model_calibration,
            random_seed=config.model.random_seed + int(fold) * 1_801,
        )
        ...
        policy_segments = rolling_ranked_policy_segments(
            prior_scored["trade_date"].astype(str).unique()
            if not prior_scored.empty
            else []
        )
        selection: RankedPolicySelection | None = None
        ...
        if policy_segments is not None:
            threshold_dates, design_dates, confirmation_dates = (
                policy_segments
            )
            threshold_calibration = prior_scored.loc[
                prior_scored["trade_date"]
                .astype(str)
                .isin(threshold_dates)
            ].copy()
            ...
            selection = select_ranked_policy(
                ...
                seed=config.model.random_seed + int(fold) * 18_007,
                bootstrap_samples=args.bootstrap_samples,
            )
            if selection.policy is not None:
                selected = apply_ranked_policy(
                    scored_test,
                    selection.policy,
                )
        ...
        test_metrics = policy_metrics(
            ...
            seed=config.model.random_seed + int(fold) * 47,
            bootstrap_samples=args.bootstrap_samples,
        )
        fold_rows.append(
            {
    ...
    assert_unique(scored_all, "V18 scored OOS frontier")
    assert_unique(selected_all, "V18 nested OOS candidates")
    policy_days = sorted(policy_evaluated_dates)
    nested_metrics = policy_metrics(
        selected_all,
        total_days=len(policy_days),
        seed=config.model.random_seed + 18_000,
    ...
    readiness = v18_research_readiness(
        nested_metrics,
        yearly=yearly,
    ...
    frontier_table = descriptive_frontier(
        scored_all,
        seed=config.model.random_seed + 180_000,
        bootstrap_samples=args.bootstrap_samples,
    )
    ...
    model_path = output / "wp_v18_shadow_model.joblib"
    joblib.dump(model_payload, model_path, compress=3)
    scored_path = atomic_write_parquet(
        scored_all,
        output / "wp_v18_selector_scored_oos.parquet",
    )
    candidate_path = atomic_write_csv(
        selected_all,
        output / "wp_v18_nested_oos_candidates.csv",
    )
    fold_path = atomic_write_csv(
        pd.json_normalize(fold_rows, sep="."),
        output / "wp_v18_nested_folds.csv",
    )
    frontier_path = atomic_write_csv(
        frontier_table,
        output / "wp_v18_frequency_profit_frontier.csv",
    )
    ...
    summary = {
        ...
        "v16_or_v17_promoted": False,
        "objective": (
            "maximize positive net T+1 close return probability for every "
            "executable 14:20-14:50 candidate while preserving a useful, "
            "predeclared candidate-day frequency"
        ),
        "source": source,
        "evaluation": {
            ...
            "model_oos_days": len(model_evaluated_dates),
            "policy_oos_days": len(policy_evaluated_dates),
        ...
        "protocol": {
            ...
            "selector": (
                "pooled regularized tree-linear selector with causal "
                "within-stock tail persistence features"
            ),
            "execution_simulation": (
                "chronological first-signal locking; later slots cannot "
                "replace an earlier candidate"
            ),
            ...
            "purge_days_between_model_segments": PURGE_DAYS,
            "purge_days_between_policy_segments": POLICY_PURGE_DAYS,
            "predeclared_policy_count": len(ranked_policy_grid()),
            "target_candidate_day_rates": [0.08, 0.12, 0.16, 0.20],
        ...
        "v17_diagnostic_reference": {
            "status": "positive_but_extremely_sparse_unconfirmed",
            "best_mean_net_return_pct": 1.863932348477506,
            "best_events": 16,
            "best_candidate_days": 7,
            "best_candidate_day_rate": 0.017114914425427872,
            "best_clustered_mean_lower_pct": -0.38543644668808114,
    ...
    atomic_write_json(output / "wp_v18_research_summary.json", summary)
    print(
        "WP_V18_RESEARCH_RESULT="
        + json.dumps(
            json_safe(
                {
                    "schema_version": SCHEMA_VERSION,
...
def descriptive_frontier(
    ...
    *,
    ...
    segments = rolling_ranked_policy_segments(
    ...
    evaluation_dates = [*design_dates, *confirmation_dates]
    evaluation = scored.loc[
        scored["trade_date"].astype(str).isin(evaluation_dates)
    ]
    rows = []
    for offset, spec in enumerate(ranked_policy_grid()):
        policy = calibrate_ranked_policy(
            threshold,
            spec,
            calibration_dates=threshold_dates,
        )
        metrics = policy_metrics(
            apply_ranked_policy(evaluation, policy),
...
def frontier_diagnostics(frontier: pd.DataFrame) -> dict[str, Any]:
    ...
    practical = frontier.loc[frequency.between(0.08, 0.30)].copy()
...
def select_final_policy(
    ...
    *,
    ...
    if segments is None:
        return None
    threshold_dates, design_dates, confirmation_dates = segments
    return select_ranked_policy(
        scored.loc[
            scored["trade_date"].astype(str).isin(threshold_dates)
        ],
        scored.loc[scored["trade_date"].astype(str).isin(design_dates)],
        scored.loc[
        ...
        design_total_days=len(design_dates),
        confirmation_total_days=len(confirmation_dates),
        seed=config.model.random_seed + 1_800_000,
        bootstrap_samples=bootstrap_samples,
    )
...
def fit_final_selector(
    ...
    *,
    ...
    if segments is None:
        raise RuntimeError("insufficient history for final V18 selector")
    ...
    bundle = fit_ranked_selector(
        ...
        random_seed=config.model.random_seed + 18_000_000,
    )
    return bundle, {
        "train_start": train_dates[0],
        "train_end": train_dates[-1],
...
def compact_selection(
    selection: RankedPolicySelection | None,
) -> dict[str, Any]:
