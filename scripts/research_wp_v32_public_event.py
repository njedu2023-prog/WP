...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run preregistered V32 public-event nested-OOS research."
        )
    )
    parser.add_argument("--config", required=True)
    parser.add_argument("--shard-dir", required=True)
    parser.add_argument("--v24-data-dir", required=True)
    parser.add_argument("--v32-data-dir", required=True)
    parser.add_argument("--v32-data-run-id", required=True, type=int)
...
def main() -> int:
    args = parse_args()
    if args.v32_data_run_id <= 0:
        raise ValueError("V32 immutable data run ID must be positive")
    ...
    event_features, data_manifest, event_integrity = (
        load_v32_event_features(args.v32_data_dir)
    )
    validate_v32_v24_source(
        args.v24_data_dir,
        data_manifest,
    )
    ...
    source = {
        **source,
        "schema_version": "wp_v32_research_source_1",
        "source_candidate_rows": int(len(source_candidates)),
    }
    assert_unique(
        source_candidates,
        "V32 immutable outcome-blind V9 top-five candidates",
    )
    joined = join_features(source_candidates, v24_features)
    joined = join_event_features(joined, event_features)
    assert_unique(joined, "V32 joined source candidates")
    ...
    if not evaluation_dates:
        raise RuntimeError("V32 evaluation window has no A-share trade dates")
    ...
    data_integrity = bool(v24_integrity and event_integrity)
    policy_spec = EventPolicySpec(
    ...
    for fold in folds:
        ...
        if overlap:
            raise RuntimeError(
                f"V32 outer test dates overlap: {sorted(overlap)[:5]}"
            )
        ...
        if segments is None:
            fold_rows.append(
                skipped_fold(
                    ...
                    test,
                    test_dates,
                    reason="insufficient_prior_oos_event_history",
                    config=config,
        ...
        train = labeled_event_rows(
            history.loc[
                history["trade_date"].astype(str).isin(train_dates)
            ]
        )
        calibration = labeled_event_rows(
            history.loc[
        ...
        if not fold_temporal:
            raise RuntimeError(
                f"V32 fold {fold} historical evidence crosses test start"
            )
        try:
            bundle = fit_event_ranker(
                ...
                calibration,
                random_seed=(
                    config.model.random_seed + int(fold) * 32_003
                ),
            )
            ...
            fold_rows.append(
                {
                    **skipped_fold(
                        ...
                        test,
                        test_dates,
                        reason="insufficient_complete_prior_event_rows",
                        config=config,
                        bootstrap_samples=args.bootstrap_samples,
                    ),
                    "temporal_integrity": fold_temporal,
            ...
            print(f"[wp-v32] fold={fold} skipped: {error}", flush=True)
            continue
        ...
        policy = calibrate_event_policy(
            scored_calibration,
            calibration_dates=calibration_dates,
            spec=policy_spec,
        )
        scored_test = bundle.predict(test)
        scored_test["v32_source_fold"] = int(fold)
        selected = apply_event_policy(scored_test, policy)
        selected["v32_source_fold"] = int(fold)
        ...
        rank_diagnostics = event_rank_diagnostics(
            scored_test,
        ...
        fold_rows.append(
            {
                **base,
                "scored": True,
                "reason": "fixed_prior_oos_v32_policy_applied",
                "temporal_integrity": fold_temporal,
    ...
    assert_unique(scored_all, "V32 nested OOS scored candidates")
    assert_unique(selected_all, "V32 nested OOS selected candidates")
    validate_selected_contract(selected_all, None)
    nested_metrics = economic_policy_metrics(
        selected_all,
        total_days=len(evaluation_dates),
        seed=config.model.random_seed + 32_000,
    ...
    yearly = yearly_metrics(
        ...
        seed=config.model.random_seed + 32,
        bootstrap_samples=args.bootstrap_samples,
    )
    add_yearly_economic_metrics(yearly, selected_all)
    rank_diagnostics = event_rank_diagnostics(
        scored_all,
        seed=config.model.random_seed + 32_032,
        bootstrap_samples=max(4_000, args.bootstrap_samples),
    )
    ...
    bundle_path = output / "wp_v32_frozen_research_bundle.joblib"
    joblib.dump(
        {
            ...
            "event_ranker": final_bundle,
            "policy": final_policy,
            "source": source,
            "v24_data_manifest": v24_manifest,
            "v32_data_manifest": data_manifest,
        },
        bundle_path,
        compress=3,
    )
    ...
    summary = {
        ...
        "new_information_family": (
            "causal A-share-normalized forecast, repurchase, share-float, "
            "and block-trade events disclosed in the five prior trade days"
        ),
        ...
        "protocol": {
            ...
            "v32_data_run_id": args.v32_data_run_id,
            ...
            "event_active_candidate_required": True,
            "event_sources": list(ADMITTED_SOURCES),
            "event_lookback_trade_days": 5,
    ...
    atomic_write_json(output / "wp_v32_research_summary.json", summary)
    atomic_write_csv(
        pd.json_normalize(fold_rows, sep="."),
        output / "wp_v32_folds.csv",
    )
    atomic_write_parquet(
        scored_all,
        output / "wp_v32_nested_oos_scored_candidates.parquet",
    )
    atomic_write_csv(
        selected_all,
        output / "wp_v32_nested_oos_candidates.csv",
    )
    atomic_write_parquet(
        selected_all,
        output / "wp_v32_nested_oos_candidates.parquet",
    )
    atomic_write_csv(
        pd.json_normalize(yearly, sep="."),
        output / "wp_v32_yearly.csv",
    )
    print(
        "WP_V32_RESULT="
        + json.dumps(
...
def load_v32_event_features(
    data_dir: str | Path,
) -> tuple[pd.DataFrame, dict[str, Any], bool]:
    root = Path(data_dir)
    manifests = list(
        root.rglob("wp_v32_public_event_data_manifest.json")
    )
    if len(manifests) != 1:
        raise RuntimeError(
            f"expected one V32 data manifest under {root}; "
            f"found {len(manifests)}"
        )
    manifest = json.loads(manifests[0].read_text(encoding="utf-8"))
    if manifest.get("schema_version") != DATA_SCHEMA_VERSION:
        raise RuntimeError("V32 data manifest schema mismatch")
    if manifest.get("profit_outcomes_read") is not False:
        raise RuntimeError("V32 data build was not outcome blind")
    if manifest.get("future_information_allowed") is not False:
        raise RuntimeError("V32 data build permitted future information")
    if not manifest.get("v32_model_research_authorized"):
        raise RuntimeError("V32 data manifest did not authorize research")
    ...
    if not (
        ...
        and coverage.get("event_coverage_gate_passed")
        and (coverage.get("probe_parity") or {}).get("passed")
        and candidate_audit.get("coverage_passed")
    ):
        raise RuntimeError("V32 event coverage contract failed")

    feature_paths = list(
        root.rglob("wp_v32_public_event_features.parquet")
    )
    index_paths = list(
        root.rglob("wp_v32_outcome_blind_candidate_index.parquet")
    )
    if len(feature_paths) != 1 or len(index_paths) != 1:
        raise RuntimeError(
            "V32 data artifact must contain one feature parquet "
            "and one candidate index"
        )
    artifacts = manifest.get("artifacts") or {}
    expected_feature_sha = str(
        (artifacts.get("event_features") or {}).get("sha256") or ""
    )
    ...
    if (
        ...
        raise RuntimeError("V32 event feature digest mismatch")
    if (
        not expected_index_sha
        or file_sha256(index_paths[0]) != expected_index_sha
    ):
        raise RuntimeError("V32 candidate-index digest mismatch")
    ...
    if missing:
        raise RuntimeError(f"V32 feature parquet missing columns: {missing}")
    identity = [*IDENTITY_COLUMNS, "fold"]
    if features.duplicated(list(IDENTITY_COLUMNS), keep=False).any():
        raise RuntimeError("V32 feature parquet has duplicate identities")
    if candidate_index.duplicated(list(IDENTITY_COLUMNS), keep=False).any():
        raise RuntimeError("V32 candidate index has duplicate identities")
    ...
    if (
        ...
        raise RuntimeError("V32 data row count changed immutable candidates")
    ...
    if not left.reset_index(drop=True).equals(right.reset_index(drop=True)):
        raise RuntimeError(
            "V32 feature identities differ from immutable candidate index"
        )
    numeric = features.loc[:, V32_EVENT_FEATURE_COLUMNS].apply(
        pd.to_numeric,
        errors="coerce",
    )
    if np.isinf(numeric.to_numpy(dtype=float)).any():
        raise RuntimeError("V32 feature parquet contains infinite values")
    required_common = [
        column
        for column in COMMON_FEATURE_COLUMNS
        if column.endswith(("_event_count_5d", "_active_5d"))
    ]
    if numeric.loc[:, required_common].isna().to_numpy().any():
        raise RuntimeError("V32 common event features are incomplete")
    for source in ADMITTED_SOURCES:
        count = numeric[f"v32_{source}_event_count_5d"]
        active = numeric[f"v32_{source}_active_5d"]
        if not active.eq(count.gt(0).astype(float)).all():
            raise RuntimeError(
                f"V32 {source} active flag disagrees with event count"
            )
...
def join_event_features(
    ...
    event = event_features.loc[
        :,
        [*identities, "fold", *V32_EVENT_FEATURE_COLUMNS],
    ].copy()
    for column in identities:
        event[column] = event[column].astype(str)
    event.rename(columns={"fold": "_v32_event_fold"}, inplace=True)
    ...
    if not result["_merge"].eq("both").all():
        raise RuntimeError("V32 event join missed source identities")
    source_fold = pd.to_numeric(result["fold"], errors="coerce")
    event_fold = pd.to_numeric(
        result["_v32_event_fold"],
        errors="coerce",
    )
    if not source_fold.equals(event_fold):
        raise RuntimeError("V32 event folds do not match immutable source")
    numeric = result.loc[:, V32_EVENT_FEATURE_COLUMNS].apply(
        pd.to_numeric,
        errors="coerce",
    )
    required_common = [
    ...
    complete = (
        numeric.loc[:, required_common].notna().all(axis=1)
        & ~np.isinf(numeric.to_numpy(dtype=float)).any(axis=1)
    )
    if not complete.all():
        raise RuntimeError("V32 event join produced incomplete features")
    result["v32_event_features_complete"] = complete
    result["v32_event_any"] = pd.concat(
        [
            numeric[f"v32_{source}_active_5d"].gt(0)
            for source in ADMITTED_SOURCES
        ],
        axis=1,
    ).any(axis=1)
    return result.drop(columns=["_v32_event_fold", "_merge"])


def validate_v32_v24_source(
    v24_data_dir: str | Path,
    v32_manifest: dict[str, Any],
) -> None:
    manifests = list(
    ...
    if len(manifests) != 1:
        raise RuntimeError(
            "V32 requires one immutable V24 data manifest"
        )
    expected_sha = str(
        (
            (v32_manifest.get("source_contract") or {})
    ...
    if not expected_sha or file_sha256(manifests[0]) != expected_sha:
        raise RuntimeError(
            "V32 event data does not match immutable V24 quality data"
        )


def event_rank_diagnostics(
    scored: pd.DataFrame,
    *,
    seed: int,
    bootstrap_samples: int,
) -> dict[str, Any]:
    if "v32_event_any" not in scored:
        raise RuntimeError("V32 rank diagnostics require event-active flags")
    diagnostic_frame = scored.loc[
        scored["v32_event_any"].fillna(False).astype(bool)
    ].rename(
        columns={
...
def fit_final_model(
    ...
    *,
    ...
    random_seed: int,
    policy_spec: EventPolicySpec,
) -> tuple[EventRankBundle, FrozenEventPolicy, dict[str, Any]]:
    segments = rolling_cross_section_segments(
        calendar_dates,
        reserve_final_purge=False,
    )
    if segments is None:
        raise RuntimeError("V32 final model has insufficient OOS history")
    train_dates, calibration_dates = segments
    train = labeled_event_rows(
        joined.loc[
    ...
    bundle = fit_event_ranker(
        train,
        calibration,
        random_seed=random_seed + 320_032,
    )
