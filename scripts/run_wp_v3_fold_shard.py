...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one deterministic shard of the WP V9 nested walk-forward backtest."
    )
...
def main() -> int:
    ...
    print(
        f"[wp-v9] shard={args.shard_index}/{args.shard_count} "
    ...
    print(
        f"[wp-v9] selective panel={panel_start}..{panel_end} "
        f"rows={len(panel):,}",
        flush=True,
    )
    ...
    print(
        f"[wp-v9] shard complete index={args.shard_index} "
