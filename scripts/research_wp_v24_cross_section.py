...
from wp.v3.v24_cross_section import (
    ...
    calibrate_cross_section_policy,
    fit_cross_section_gate,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the preregistered V24 cross-section microstructure model "
            "over the immutable V9 top-five opportunity set."
        )
    )
...
def main() -> int:
    ...
    features, data_manifest, data_integrity = load_v24_features(
        args.data_dir,
    )
    ...
    source = {
        **source,
        "schema_version": "wp_v24_research_source_1",
        "source_candidate_rows": int(len(source_candidates)),
    }
    assert_unique(
        source_candidates,
        "V24 immutable outcome-blind V9 top-five candidates",
    )
    joined = join_features(source_candidates, features)
    assert_unique(joined, "V24 joined source candidates")
    ...
    if not set(
        ...
        raise RuntimeError("V24 source dates are outside the trade calendar")
    if not evaluation_dates:
        raise RuntimeError("V24 evaluation window has no A-share trade dates")
    data_integrity &= bool(
    ...
    policy_spec = CrossSectionPolicySpec(
    ...
    for fold in folds:
        ...
        if overlap:
            raise RuntimeError(
                f"V24 outer test dates overlap: {sorted(overlap)[:5]}"
            )
            ...
            fold_rows.append(
                {
                    **base,
                    "scored": False,
                    "reason": "insufficient_prior_oos_cross_section_history",
                    "selected": metrics,
                }
            )
            continue
        ...
        if not fold_temporal:
            raise RuntimeError(
                f"V24 fold {fold} historical evidence crosses test start"
            )
        if (
        ...
        policy = calibrate_cross_section_policy(
            scored_calibration,
            calibration_dates=calibration_dates,
            spec=policy_spec,
        )
        scored_test = bundle.predict(test)
        scored_test["v24_source_fold"] = int(fold)
        selected = apply_cross_section_policy(scored_test, policy)
        selected["v24_source_fold"] = int(fold)
        ...
        fold_rows.append(
            {
                **base,
                "scored": True,
                "reason": "fixed_prior_oos_v24_policy_applied",
                "temporal_integrity": fold_temporal,
                "model": {
                    "train_start": train_dates[0],
                    "train_end": train_dates[-1],
    ...
    assert_unique(scored_all, "V24 nested OOS scored candidates")
    assert_unique(selected_all, "V24 nested OOS candidates")
    ...
    final_bundle_path = output / "wp_v24_frozen_research_bundle.joblib"
    joblib.dump(
        {
            ...
            "cross_section_gate": final_bundle,
            "policy": final_policy,
            "source": source,
            "data_manifest": data_manifest,
        },
    ...
    summary = {
        ...
        "new_information_family": (
            ...
            "previous-trading-day L2 money flow, and outcome-blind "
            "cross-sectional source features"
        ),
        "execution_contract": {
    ...
    atomic_write_json(output / "wp_v24_research_summary.json", summary)
    atomic_write_csv(
        pd.json_normalize(fold_rows, sep="."),
        output / "wp_v24_folds.csv",
    )
    atomic_write_parquet(
        scored_all,
        output / "wp_v24_nested_oos_scored_candidates.parquet",
    )
    atomic_write_csv(
        selected_all,
        output / "wp_v24_nested_oos_candidates.csv",
    )
    atomic_write_parquet(
        selected_all,
        output / "wp_v24_nested_oos_candidates.parquet",
    )
    atomic_write_csv(
        pd.json_normalize(yearly, sep="."),
        output / "wp_v24_yearly.csv",
    )
    print(
        "WP_V24_RESULT="
        + json.dumps(
...
def load_v24_features(
    ...
    if len(manifests) != 1:
        raise RuntimeError(
            f"expected one V24 data manifest under {root}; found {len(manifests)}"
        )
    manifest = json.loads(manifests[0].read_text(encoding="utf-8"))
    if manifest.get("schema_version") != DATA_SCHEMA_VERSION:
        raise RuntimeError("V24 data manifest schema mismatch")
    if manifest.get("profit_outcomes_read") is not False:
        raise RuntimeError("V24 data build was not outcome blind")
    if not manifest.get("v24_model_research_authorized"):
        raise RuntimeError("V24 data manifest did not authorize model research")
    ...
    if (
        ...
        raise RuntimeError("V24 data coverage contract failed")
    feature_paths = list(root.rglob("wp_v24_point_in_time_features.parquet"))
    if len(feature_paths) != 1:
        raise RuntimeError(
            f"expected one V24 feature parquet; found {len(feature_paths)}"
        )
    expected_sha = str(
    ...
    if not expected_sha or actual_sha != expected_sha:
        raise RuntimeError("V24 feature digest mismatch")
    ...
    if missing:
        raise RuntimeError(f"V24 feature parquet missing columns: {missing}")
    if features.duplicated(
        ["trade_date", "signal_slot", "ts_code"],
        keep=False,
    ).any():
        raise RuntimeError("V24 feature parquet contains duplicate identities")
    expected_rows = int(
        (manifest.get("requirements") or {}).get("candidate_rows", -1)
    )
    if expected_rows != len(features):
        raise RuntimeError(
            "V24 feature row count does not match the data manifest"
        )
...
def join_features(
    ...
    feature_frame.rename(columns={"fold": "_v24_data_fold"}, inplace=True)
    joined = source.merge(
        feature_frame,
    ...
    joined_fold = pd.to_numeric(joined["_v24_data_fold"], errors="coerce")
    if not source_fold.reset_index(drop=True).equals(
        joined_fold.reset_index(drop=True)
    ):
        raise RuntimeError("V24 feature folds do not match V9 source folds")
    if joined["v23_point_in_time_complete"].isna().any():
        raise RuntimeError("V24 feature join is missing source identities")
    return joined.drop(columns="_v24_data_fold")
...
def fit_final_model(
    ...
    *,
    ...
    if segments is None:
        raise RuntimeError("V24 final model has insufficient OOS history")
    train_dates, calibration_dates = segments
    train = labeled_complete_rows(
    ...
    if (
        ...
        raise RuntimeError(
            "V24 final model lacks the preregistered complete-row sample: "
            f"train={len(train)}/{MINIMUM_TRAIN_ROWS} "
            f"calibration={len(calibration)}/{MINIMUM_CALIBRATION_ROWS}"
        )
    bundle = fit_cross_section_gate(
        train,
        calibration,
