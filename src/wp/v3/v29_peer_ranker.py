...
from wp.v3.meta_alpha import ProbabilityCalibrator
...
SCHEMA_VERSION = "wp_v29_hierarchical_peer_ranker_1"
...
MODEL_FEATURES = tuple(V29_FEATURE_COLUMNS)
MODEL_FEATURE_SET = frozenset(MODEL_FEATURES)


@dataclass(frozen=True)
class PeerPolicySpec:
    ...
    def policy_id(self) -> str:
        return (
            f"v29-peer-rate{self.target_candidate_day_rate:.2f}-"
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "model_probability_hard_gates": False,
            "first_signal_is_immutable": True,
        }


@dataclass(frozen=True)
class FrozenPeerPolicy:
    spec: PeerPolicySpec
...
class PeerRankBundle:
    ...
    def predict(self, frame: pd.DataFrame) -> pd.DataFrame:
        result = frame.copy()
        matrix = peer_feature_matrix(result, self.feature_columns)
        result["v29_p_positive"] = calibrated_probability(
            self.positive_model,
            self.positive_calibrator,
            matrix,
        )
        result["v29_p_margin"] = calibrated_probability(
            self.margin_model,
            self.margin_calibrator,
            matrix,
        )
        result["v29_p_severe_loss"] = calibrated_probability(
            self.severe_model,
            self.severe_calibrator,
            matrix,
        )
        result["v29_within_slot_rank_score"] = predict_pairwise_scores(
            result,
            feature_columns=self.feature_columns,
            model=self.pairwise_model,
            calibrator=self.pairwise_calibrator,
        )
        result["v29_peer_score"] = (
            POSITIVE_SCORE_WEIGHT * (result["v29_p_positive"] - 0.50)
            + MARGIN_SCORE_WEIGHT * (result["v29_p_margin"] - 0.35)
            + PAIRWISE_SCORE_WEIGHT
            * (result["v29_within_slot_rank_score"] - 0.50)
            - SEVERE_SCORE_WEIGHT * result["v29_p_severe_loss"]
        )
        return result
...
def fit_peer_ranker(
    ...
    *,
    ...
    prepared_train = labeled_peer_rows(train)
    prepared_calibration = labeled_peer_rows(calibration)
    if len(prepared_train) < minimum_train_rows:
        raise ValueError(
            f"V29 has {len(prepared_train)} train rows; "
            f"requires {minimum_train_rows}"
        )
    if len(prepared_calibration) < minimum_calibration_rows:
        raise ValueError(
            f"V29 has {len(prepared_calibration)} calibration rows; "
            f"requires {minimum_calibration_rows}"
        )
    features = active_peer_features(prepared_train, prepared_calibration)
    if len(features) < 20:
        raise ValueError(f"V29 has only {len(features)} active features")

    train_matrix = peer_feature_matrix(prepared_train, features)
    calibration_matrix = peer_feature_matrix(
    ...
    for offset, name in enumerate(("positive", "margin", "severe"), start=1):
        target = targets[name]
        if target.nunique() < 2:
            raise ValueError(f"V29 {name} target lacks both classes")
    ...
    if len(train_pairs) < minimum_train_pair_rows:
        raise ValueError(
            f"V29 has {len(train_pairs)} train pair rows; "
            f"requires {minimum_train_pair_rows}"
        )
    if len(calibration_pairs) < minimum_calibration_pair_rows:
        raise ValueError(
            f"V29 has {len(calibration_pairs)} calibration pair rows; "
...
def labeled_peer_rows(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    peer_complete = _boolean(frame, "v29_peer_features_complete")
...
def calibrate_peer_policy(
    scored_calibration: pd.DataFrame,
    *,
    calibration_dates: Iterable[str],
    spec: PeerPolicySpec | None = None,
) -> FrozenPeerPolicy:
    frozen_spec = spec or PeerPolicySpec()
    dates = sorted(set(map(str, calibration_dates)))
    if not dates:
        raise ValueError("V29 policy calibration has no dates")
    eligible = peer_slot_leaders(policy_eligible_rows(scored_calibration))
    daily_max = (
        eligible.groupby("trade_date", sort=False)["v29_peer_score"]
        .max()
...
def apply_peer_policy(
    ...
    eligible = peer_slot_leaders(policy_eligible_rows(scored))
    qualified = eligible.loc[
        _numeric(eligible, "v29_peer_score").ge(policy.score_threshold)
    ].copy()
    if qualified.empty:
        qualified["v29_policy_id"] = policy.policy_id
        return qualified
    ...
    selected["v29_policy_id"] = policy.policy_id
    selected["v29_score_threshold"] = policy.score_threshold
...
def policy_eligible_rows(scored: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"V29 policy frame missing columns: {missing}")
    ...
    eligible = (
        ...
        & _boolean(scored, "v29_peer_features_complete")
...
def peer_slot_leaders(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.empty:
        return frame.copy()
...
def validate_selected_contract(
    ...
    policy: FrozenPeerPolicy | None,
) -> None:
    if selected.empty:
        return
    if selected.duplicated(["trade_date", "ts_code"], keep=False).any():
        raise RuntimeError("V29 selected output rewrote a first signal")
    maximum = (
    ...
    if int(selected.groupby("trade_date").size().max()) > maximum:
        raise RuntimeError("V29 selected output exceeds fixed daily maximum")
    ...
    if not slot.between("1420", "1450").all():
        raise RuntimeError("V29 selected output contains an illegal slot")
    if not _boolean(selected, "v23_point_in_time_complete").all():
        raise RuntimeError("V29 selected output contains incomplete PIT data")
    if not _boolean(selected, "v29_peer_features_complete").all():
        raise RuntimeError("V29 selected output contains incomplete peer data")
