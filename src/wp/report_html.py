...
def _decision_support_panel(decision: dict, regime: dict) -> str:
    ...
    return (
        ...
        f"<span>候选截止 <strong>{html.escape(str(decision.get('candidate_deadline') or '14:50'))}</strong></span>"
        f"<span>人工买入截止 <strong>{html.escape(str(decision.get('entry_deadline') or '15:00'))}</strong></span>"
