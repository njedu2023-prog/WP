                ...
                rows.append(
                    {
                        ...
                        "expected_net_return_pct": -0.8 + 2.0 * score,
                        ...
                        "passes_expected_return": score >= 0.55,
                        "passes_downside": True,
                        "passes_sample": True,
                        "passes_empirical_lower": score >= 2 / 3,
...
def test_prediction_diagnostics_exposes_discrimination_and_policy_funnel():
    ...
    assert diagnostics["score_quality"]["probability"]["roc_auc"] > 0.99
    assert diagnostics["score_deciles"]
    probability_top_one = next(
        row
        for row in diagnostics["top_n_per_slot"]
        if row["score"] == "probability" and row["top_n"] == 1
    )
